...
import gyum_02.scheduler.controller.*;
...
public class SchedulerController {
   ...
   @RequestMapping("/study_schedule.do")
   public String scheduler(ModelMap model, HttpServletRequest request) throws Exception {
	   ...
	   planner.data.dataprocessor.setURL("events.do?study_idx="+study_idx);
   ...
   private String saveOne(HttpServletRequest request, String id, String prefix)
   {
       ...
       if(security.can(status).booleanValue())
       {
           ...
           status = saveEvent(ev, status,request);
       } else
       {
   ...
   private String toLowerCase(String value)
   {
  	...
  	public DHXEv createEvent(String id, DHXStatus status) {
   	...
   	public DHXStatus saveEvent(DHXEv event, DHXStatus status,HttpServletRequest request) {
   		HttpSession session=request.getSession();
        ...
        if (status == DHXStatus.UPDATE) {
        	...
        	schedulervo.setStudy_event_dayrepot(0);
        	eventManagerDao.insertEvent(schedulervo);

...
public class EchoHandler extends TextWebSocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        logger.info(map.get("name") + "님이 접속했습니다.");
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
                ...
                if (!session.getId().equals(webSocketSession.getId())) {
                    webSocketSession.sendMessage(
                            new TextMessage(
                            		...
                            		+ map.get("name") + " ▶ " + messageVO.getMessage()
                            		+ "</span>"));
                }
            } else {
         ...
         * Payload : 사용자가 보낸 메시지
         */
        logger.info(map.get("name") + "님의 메시지 : " + message.getPayload());
    }
    ...
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
        ...
        for (WebSocketSession webSocketSession : connectedUsers) {
             ...
             */
            if (!session.getId().equals(webSocketSession.getId())) {
                webSocketSession.sendMessage(new TextMessage(map.get("name") + "님이 퇴장했습니다."));
            }
        }
 
        logger.info(map.get("name") + "님이 퇴장했습니다.");
    }
}

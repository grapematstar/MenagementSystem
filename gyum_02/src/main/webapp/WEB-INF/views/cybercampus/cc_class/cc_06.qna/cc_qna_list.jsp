...
<section>
		...
		<div class="panel">
			<div class="panel-body">
				...
				<c:if test="${sessionScope.usergrade=='teacher'||sessionScope.usergrade=='master'}">
                   ...
                   <li><a href="cc_gotoQnAList.do" class="btn btn-outline btn-primary">질의응답</a></li>
	...
	<c:forEach var="list" items="${list}">
		<c:url var="cc_gotoQnAContent" value="cc_gotoQnAContent.do">
			<c:param name="c_q_q_idx" value="${list.c_q_q_idx}"/>
		</c:url>
		<tr>
			<td>${list.c_q_q_idx}</td>
			<td><a href="${cc_gotoQnAContent}">${list.c_q_q_subject}</a></td>
			<td>${list.c_q_q_writer}</td>
			<td>${list.string_Qwritedate}</td>
		</tr>
	</c:forEach>
	</tbody>
	<tfoot>
...
<div style="text-align:right;"><input type="button" class="btn btn-default" value="글쓰기" onclick="location.href='cc_gotoQnAWrite.do'"></div>
							</div>
							</div>
						</div>
					</div>

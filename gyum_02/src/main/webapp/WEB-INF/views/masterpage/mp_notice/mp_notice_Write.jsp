...
<div id="content" class="article-v1">
	<section id="whole">
		<section id="inwhole">
			<form name="noticeWriteform" id="noticeWriteform" action="notice_Write.do" method="post">
				<div id="nobtn">
				...
				<table id="tab">
					<tr>
						...
						<td><input type="text" id="notice_subject" name="notice_subject" value="${dto.notice_subject}"></td>
					</tr>
					<tr>
						<td colspan="2" id="edittool">
							<span id="writediv" class="col-md-12 col-sm-12 col-xs-12 mail-reply">
				            	<textarea class="summernote" name="notice_content" id="notice_content">${dto.notice_content}</textarea>
   ...
   <script type="text/javascript">
     ...
     <script>
     	function backtotheindex() {
     		location.href="notice_go.do"
     	}
     	
     </script>
     
     <script>
     if(${!empty dto.notice_idx}){
 		var a = document.getElementById('noticeWriteform');
 		document.all.notice_idx.setAttribute('name', 'notice_idx')
 		a.setAttribute('action', 'notice_Update.do')
 		var b = document.getElementById('okbtn');
 		console.log(b.value);
 		b.setAttribute('value', '수정');
 		console.log(b.value);
 		var s1 = document.all.nobtn;
 		s1.innerHTML = '<a href="noticeDel.do?idx=${dto.notice_idx}&sel=notice_">삭제하기</a>';
 	}
     </script>
</html>